...
- **Level 11 — Michigan Wilds**: Heightmap terrain side-zone at `(3000, 0, 1500)` using `MIHEIGHTMAP.png` served as `/textures/miheightmap.png`. Built with Babylon `CreateGroundFromHeightMap` and `TerrainMaterial`: low elevations become flooded lowlands under a sea-level water plane, mid elevations blend to grass foothills, and high elevations blend to generated rock texture. It hides the existing city/mountain/foliage props on mount so Detroit city materials remain untouched. Encounter layer: rare/legendary bio-creatures, collectible power blooms, rogue labs with L11 rescue cages, giant base landmarks, mothership wreck/patrol visuals, and new active `wilds_titan` / `wilds_transformer` enemy classes mixed with existing villains and ships. **Realistic nature**: along with Ashur Sanctuary, this is one of only two zones dressed as real Earth woodland. `buildEarthForest()` forests the green terrain tiers via `EarthFoliageSystem.scatterPoints` — groves + lone trees keyed by `classifyTile` (rejects water `y<SEA+1.2`, bare rock `y>52`, cliffs `slope>0.9`) with willows on the shore, oak/birch in lowlands, pine on foothills, sparse pine on the subalpine fringe; per-plant Y from `getHeightAt`. `buildShorelineProps()` adds decorative rock/log meshes (isPickable=false, existing rock/wood textures) near shoreline/lowlands. Both seed once in `seedTerrainContentOnce` after the heightmap is ready; the forest disposer + root-parented props are cleaned on warp-out.
- **Level 4 — Ashur Sanctuary**: Peaceful side-zone now uses a real rolling `sanctuaryTerrain` mesh, an expanded village/house kit, a visible pet clinic/hospital, farm harvests that can produce Animaton Feed, and pet-care bonding from the Bio Garden. Direct fast-travel from Ashur to Detroit L1/L2/L3 is blocked; route out through another outpost/wilds hop. **Realistic nature**: along with Michigan Wilds, this is one of only two zones dressed as real Earth woodland — it scatters oak/birch/willow/shrub/fern via the shared `EarthFoliageSystem` (NOT the alien glow-foliage) in two bands around the village (`scatterEarthFoliage`, y=0), and the wetland pond uses a calmer natural tone. Trees are wiped on warp-out via the returned disposer.
...
- **Mega Man player look vs captain kit**: The player is dressed by the modular `mm_*` armor set (`DEFAULT_ARMOR_SET` in `RobotArmorSystem`), and `createPlayerMesh` sets `hasArmor: false` so `HumanoidCharacter.buildArmor`'s `armorType: "captain"` kit never runs for the player. Refining the `mm_*` parts changes ONLY the player; refining the captain branch changes ONLY captains/commanders/General. Both reuse the shared humanoid materials, so the boss-variant tint loop still recolors captains. Titans (`titan`/`wilds_titan`) are a separate path — dedicated `MegaTitan*`/`MegaTankTitan` presets in `RobotPresets`, NOT boss-tinted.
- **Swim exit needs the grace timer**: The player leaves water two ways — holding `SPACE` (the swim ceiling `maxBodyY` sits above the exit threshold) or a discrete `SPACE` tap that calls `tryJumpOutOfWater()`. The tap launch only survives because a short `waterExitGraceTimer` (~0.4 s) suppresses swim re-entry + the jetpack cap; without it, buoyancy snaps the player back to the surface. Both exits drop into the normal jump/airborne flow, so double-jump/flight stay available.
- **Hit pipeline must not allocate**: The per-frame target list in `Game.tsx` is one reused `enemyMeshScratch` array; each source system `append*`s its live meshes into it. NEVER `scratch.push(...getWhatever())` — the spread re-adds per-frame garbage and risks a stack overflow at high target counts. Rebuild the scratch BEFORE the melee updates that read it (BeamSabre / MeleeArsenal target providers hand back the same array). `ELEMENTAL_DEFINITIONS` are `Object.freeze`d and the elemental getters return the shared frozen `Readonly` ref — treat as immutable; clone at the call site if you need a mutable copy.
