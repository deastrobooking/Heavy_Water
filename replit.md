...
### Enemy Systems
The EnemySystem features a wave spawner for distinct enemy types, including Commanders. The Robot Shape Engine generates all robots. Aerial enemies have specialized behaviors and engage upon player aggression. Hostile Enemy Bases include turrets and destructible loot vaults. **Aerial regroup** uses two independent 5-minute timers in `AerialEnemySystem`: `fortressRegroupTimer` (gates only Flying-Fortress respawns) and `skiesClearedTimer` (gates ALL aerial spawns — fighters, battleships, AND fortresses — once the player clears every live aerial unit). Both fire on a "was-alive → now-zero" edge transition, so they don't trigger at game start.

BossCaptain is a humanoid boss enemy mirroring the player's kit, spawned in specific scenarios. **The Boss Fortress** is the Level-1 capstone: 12 high-HP turrets (`BOSS_TURRET_HP=280`) ringing a central command spire whose glowing red core is the kill target. Spire vault uses `BOSS_SPIRE_HP=900` (tuned down from the original 1800 — players were getting stuck and never seeing Level 2). When all turrets fall: `BOSS_FORTRESS_TURRETS_CLEARED` fires, BossCaptain spawns at the spire entrance, the spire becomes `armed` (gold), and the HUD prints two telegraph lines ("FORTRESS DEFENSES DOWN — BOSS CAPTAIN INCOMING" then "FIRE AT THE GLOWING RED CORE — SPIRE VULNERABLE"). When spire HP hits 0: `crackVault` emits `BOSS_FORTRESS_CLEARED` and frees the captured ally, which is what `LevelSystem` listens for to advance Level 1 → Level 2.
