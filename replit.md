...
PlayerController manages humanoid characters with complex state machines for movement, combat, and a triple-jump flight system. Rocket Skates engage after a short sprint. The HumanoidCharacter system enables procedural generation, modular body parts, and customization. An AnimationSystem provides procedural, multi-part animations. The CharacterEditor allows player customization, sharing the same humanoid + Mega Man-style armor pipeline as the in-game player: it instantiates a `HumanoidCharacter` from `HUMANOID_PRESETS.PlayerDefault` (so `visualScale: 0.12` and the Mega Man armor frame are the defaults), equips parts via `equipArmorSet` against `ARMOR_PART_REGISTRY` (which merges the Mega Man parts pack), and exposes a body-tab Armor Frame selector (megaman / light / heavy / captain) plus "MEGA MAN" / "TITAN" preset buttons that swap `DEFAULT_ARMOR_SET` / `TITAN_ARMOR_SET`. Shield, stamina, and cooldown timers are persistent. A Boost Dash provides i-frames and a short burst of speed, with specific integration for a "dash → slash" combo.

### Robot and Armor Systems
The ArmorMaterialFactory creates reusable materials, and RobotArmorParts provides a data-driven registry of parametric armor parts. A "Mega Man-style" robot builder is implemented, offering specific armor parts and a default armor set. Humanoid visual scale is adjusted to fit collision capsules.
